...
Maildev находится в отдельной директории `maildev/` и деплоится как отдельный проект/сервис на Railway.
...
### 4. Создание отдельного проекта Maildev

**Вариант 1: Отдельный проект (рекомендуется)**

1. Создайте **новый проект** на Railway для Maildev
2. Нажмите **"+ New"** → **"GitHub Repo"**
3. Выберите репозиторий `av-backend`
4. В настройках репозитория укажите:
   - **Root Directory**: `maildev/`
   - Railway автоматически найдет `Dockerfile` в этой директории

**Вариант 2: Сервис в том же проекте**

1. В существующем проекте Railway нажмите **"+ New"** → **"Empty Service"**
2. Назовите сервис `maildev`
3. В настройках сервиса:
   - **Source**: выберите тот же репозиторий `av-backend`
   - **Root Directory**: укажите `maildev/`
   - Railway автоматически найдет `Dockerfile` в этой директории
